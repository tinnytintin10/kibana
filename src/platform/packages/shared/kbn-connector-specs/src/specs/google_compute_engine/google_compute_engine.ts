...
const trimOperation = (operation: ComputeOperation, zone?: string) => ({
  ...
  errors: (operation.error?.errors ?? []).map((entry) => ({
    code: entry.code,
...
Gotchas:
- EVERY mutating action here is asynchronous. It returns an operation, not a result, so a success response means "accepted", not "done". Poll getOperation until done is true before you claim a VM is stopped or a rule is live.
  ...
  actions: {
    ...
    stopInstance: {
      ...
      description:
        ...
        'Asynchronous: returns an operation, so poll getOperation with the zone until done is true before reporting the VM as stopped. ' +
    ...
    startInstance: {
      ...
      description:
        ...
        'Asynchronous: poll getOperation with the zone until done is true.',
      input: InstanceRefInputSchema,
      handler: async (ctx, input: InstanceRefInput) => {
        try {
          const response = await ctx.client.post(
    ...
    setInstanceTags: {
      ...
      handler: async (ctx, input: SetInstanceTagsInput) => {
        try {
          ...
          const response = await ctx.client.post(
            `${instancePath(input.projectId, input.zone, input.instanceName)}/setTags`,
            { items: nextTags, fingerprint }
          );
          return {
            changed: true,
    ...
    setInstanceLabels: {
      ...
      handler: async (ctx, input: SetInstanceLabelsInput) => {
        try {
          const instance = await readInstance(ctx, input.projectId, input.zone, input.instanceName);
          const response = await ctx.client.post(
            `${instancePath(input.projectId, input.zone, input.instanceName)}/setLabels`,
            { labels: input.labels, labelFingerprint: instance.labelFingerprint }
          );
          return {
    ...
    createSnapshot: {
      ...
      description:
        ...
        'Asynchronous: poll getOperation with the zone until done is true before deleting anything.',
    ...
    getOperation: {
      ...
      description:
        ...
        'Every mutating action in this connector returns an operation name, so poll this until done is true before reporting that a VM was stopped or a rule is live. ' +
