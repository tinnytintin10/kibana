...
describe('GoogleComputeEngine', () => {
  ...
  describe('listInstances', () => {
    it('uses the aggregated endpoint and flattens the zone map when no zone is given', async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        `${API}/projects/${PROJECT}/aggregated/instances`,
        { params: { filter: undefined, maxResults: undefined, pageToken: undefined } }
      );
    ...
    it('uses the zonal endpoint when a zone is given', async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        `${API}/projects/${PROJECT}/zones/${ZONE}/instances`,
        { params: { filter: 'status = "RUNNING"', maxResults: 10, pageToken: undefined } }
      );
    });
  ...
  describe('firewalls', () => {
    it('lists rules from the global collection', async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(`${API}/projects/${PROJECT}/global/firewalls`, {
        params: { filter: undefined, maxResults: undefined, pageToken: undefined },
      });
